jQuery(document).ready(function ($) {
    ...
    function showStep(step, layer) {
        ...
        console.log(step, layer);
        var positionElement = step.attr('id');
        var _target = $('#' + positionElement);
        var position = _target.offset();

        if (step.find('.bottom').length) {
            console.log('bottom');
            position.top = position.top + _target.height();
            position.left = position.left + (_target.width() / 2);
        } else if (step.find('.right').length) {
            console.log('right');
            position.top = position.top + (_target.height() / 2);
            position.left = position.left + (_target.width());
        } else if (step.find('.top').length) {
            console.log('top');
            position.top = position.top - 5;
            position.left = position.left + (_target.width() / 2);
        } else if (step.find('.left').length) {
            console.log('left');
            position.top = position.top + (_target.height() / 2);
