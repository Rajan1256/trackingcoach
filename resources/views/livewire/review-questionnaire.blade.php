                        ...
                        <div class="relative flex space-x-3 group">
                            ...
                            <div class="min-w-0 flex-1 pt-1.5">
                                <div class="flex justify-between space-x-4">
                                    <div>
                                        <p class="text-sm text-gray-500">{{ $question->model->getLatestVersion()->getTranslation('name', Auth::user()->locale) }}</p>
                                    </div>
                                        ...
                                        <x-components::grid cols="2" class="opacity-0 group-hover:opacity-100">
                                            ...
                                            <x-components::grid.block>
                                                <x-components::lists.button
                                                        py="0" px="2"
                                                        delete
                                                        href="{{ route('customers.reviews.questions.destroy', [$customer, $question->model->id]) }}">
                                ...
                                @if($question->model->id === $editingQuestion)
                                    <div>
                                        <x-components::form method="post"
                                                            action="{{ route('customers.reviews.questions.update', [$customer, $question->model]) }}">
                                                    ...
                                                    <div class=" grid grid-cols-1 divide-y gap-4 divide-dashed mb-8">
                                                        ...
                                                        <x-components::form.group-top>
                                                            ...
                                                            <x-components::form.input
                                                                    :translatable="true"
                                                                    :translations="$question->model->getLatestVersion()->getTranslations('name')"
                                                                    name="name">
                                                            </x-components::form.input>
                                                        </x-components::form.group-top>
                                                        ...
                                                        <x-components::form.group-top>
                                                            <x-components::form.label
                                                                    :translatable="true"
                                                                    for="description">{{ __('Description') }}</x-components::form.label>
                                                            <x-components::form.textarea
                                                                    :translatable="true"
                                                                    :translations="$question->model->getLatestVersion()->getTranslations('description')"
                                                                    name="description">{{ $question->model->getLatestVersion()->getTranslation('description', Auth::user()->locale) }}</x-components::form.textarea>
                                                        </x-components::form.group-top>
                                                    </div>
                ...
                <div class="relative pb-8">
                    <div class="relative flex space-x-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <div>
                                <input type="text" class="rounded py-2 px-3 w-full"
                                       placeholder="Add a new question ({{ Str::upper(Auth::user()->locale) }})"
                            ...
                            @if($templates->count() > 0)
                                @foreach($templates as $template)
                                    <x-components::form
                                            :buttonPadding="false"
                                            action="{{ route('customers.reviews.questions.template', [$customer, $template]) }}"
                                            method="post">
                                        <x-components::form.button-group>
                                            <x-components::form.button
                                                    submit
                                                    class="mx-0 ml-0 my-2 relative col-span-1 rounded-lg border border-gray-300 bg-white shadow-sm px-6 py-4 cursor-pointer hover:border-gray-400 sm:flex sm:justify-between focus:outline-none hover:bg-white w-full text-left">{{ $template->getLatestVersion()->name }}</x-components::form.button>
                                        </x-components::form.button-group>
                                    </x-components::form>
                                @endforeach
                            @endif
                            ...
                            @if($newQuestion)
                                ...
                                @if($newType)
                                    <x-components::form method="post"
                                                        action="{{ route('customers.reviews.questions.store', $customer) }}">
                                        <x-components::form.fieldset>
                                            <input type="hidden" name="name[{{ Auth::user()->locale }}]" value="{{ $newQuestion }}">
                                            ...
                                            <div class="rounded-lg border border-gray-300 bg-white shadow-sm px-6 py-4">
                                                <div class=" grid grid-cols-1 divide-y gap-4 divide-dashed mb-8">
                                                    <x-components::form.group-top>
                                                        <x-components::form.label
                                                                :translatable="true"
                                                                :defaultLanguage="collect(array_keys(config('trackingcoach.languages')))->filter(fn($l) => $l !== Auth::user()->locale)->first()"
                                                                :languages="collect(array_keys(config('trackingcoach.languages')))->filter(fn($l) => $l !== Auth::user()->locale)->toArray()"
                                                                for="name"
                                                                :small="false">{{ __("Question") }}</x-components::form.label>
                                                        <x-components::form.input
                                                                :translatable="true"
                                                    ...
                                                    <x-components::form.group-top>
                                                        ...
                                                        <x-components::form.textarea
                                                                :translatable="true"
                                                                :translations="[]"
                                                                name="description"></x-components::form.textarea>
                                                    </x-components::form.group-top>
                                                </div>
                                                <div class="flex items-center">
                                                    <x-components::form.button primary ml="0"
