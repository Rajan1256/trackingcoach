<x-app-layout>
    ...
    <div class="px-6">
        <x-components::form method="post" action="{{ route('faq.manage.store') }}" enctype="multipart/form-data">
            <x-components::form.fieldset>
                <x-components::form.group-top>
                    <x-components::form.label>{{ __('Question') }}</x-components::form.label>
                    <x-components::form.input name="question" value="{{ old('question') }}"/>
                </x-components::form.group-top>
                <x-components::form.group-top>
                    <x-components::form.label>{{ __('Answer') }}</x-components::form.label>
                    <x-components::form.trix name="answer" id="answer">{{ old('answer') }}</x-components::form.trix>
                </x-components::form.group-top>
            </x-components::form.fieldset>
