<x-app-layout>
    <x-slot name="header">
        <x-page-header>
            ...
            <x-slot name="suffix">{{ __('Edit mini-survey goal') }}</x-slot>
        </x-page-header>
    </x-slot>
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="px-6 bg-white border-b border-gray-200">
            ...
            <x-components::form action="{{ route('customers.goals.update', [$customer, $goal]) }}"
                                method="post">
                    ...
                    @foreach (config('trackingcoach.languages') as $short => $full)
                        <x-components::form.group-top>
                            ...
                            <x-components::form.input id="name[{{ $short }}]" name="name[{{ $short }}]"
                                                      value="{{ old('name['.$short.']', $goal->getTranslation('name', $short, false)) }}"/>
                        </x-components::form.group-top>
                    @endforeach
                </x-components::form.fieldset-top>
