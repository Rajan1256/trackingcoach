<x-app-layout>
    <x-slot name="header">
        <x-page-header>
            ...
            <x-slot name="suffix">{{ __('Create note') }}</x-slot>
            <x-slot name="actions">
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="px-6 bg-white border-b border-gray-200">
            <x-components::form action="{{ route('customers.notes.store', [$customer]) }}"
                                method="post">
                <x-components::form.fieldset>
                    <x-components::form.group-top>
                        <x-components::form.textarea
                                ...
                                name="note"></x-components::form.textarea>
                    </x-components::form.group-top>
                    <x-components::form.group-top>
                        ...
                        <x-components::form.select name="authorization">
                            ...
                            <x-components::form.select-option
                                    value="2">{{ __('Just me') }}</x-components::form.select-option>
                        </x-components::form.select>
                    </x-components::form.group-top>
                </x-components::form.fieldset>
                <x-components::form.button-group>
                    <x-components::form.button submit
                                               primary>{{ __('Create note') }}</x-components::form.button>
                </x-components::form.button-group>
            </x-components::form>
        </div>
    </div>
