...
use App\Models\User;
use App\Questions\FollowUp;
use App\Questions\Open;
use App\Questions\YesNoReview;
use App\Questions\ZeroCenteredSevenScale;
...
class ReviewQuestionnaire extends Component
{
    ...
    public function updateQuestionOrder($order)
    {
        ...
        $this->customer->questions()->review()->sorted()->get()->each(function (Question $question) use (
            $newOrder
        ) {
    ...
    public function render()
    {
        $this->questionTypes = collect([
            ...
            new Open,
            new YesNoReview,
            new ZeroCenteredSevenScale,
        ])->keyBy(fn($questionType) => get_class($questionType));

        return view('livewire.review-questionnaire')
            ->with([
                'customer'  => $this->customer,
                'questions' => $this->customer->questions()->review()->sorted()->get()->map(function (Question $model
                ) {
